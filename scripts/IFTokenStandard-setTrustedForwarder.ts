...
export async function main(): Promise<void> {
  ...
  const trustedForwarder: string = process.env.TRUSTED_FWDER || "" // trusted forwarder

  // get contract from token
  const tokenContract = new hre.ethers.Contract(token, IFTokenStandard.abi)

  // mint
  const result = await tokenContract.connect((await hre.ethers.getSigners())[0]).setTrustedForwarder(trustedForwarder)
