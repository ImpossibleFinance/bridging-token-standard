...
export default describe('test ImpossibleAdapter', async () => {
  ...
  beforeEach(async () => {
    ...
    ifAdapter = []
    ifAdapter.push(await AdapterFactory.deploy(
        "AnyTestToken",
        "AnyTT",
        underlying.address,
        Mode.LOCK,
  ...
  it('test dynamic quota', async () => {
    ...
    for (let i: number = 0; i < 2; i++) {
      ...
      for (let j: number = 0; j < tests.length; j++) {
        ...
        remainder = remainder.sub(min(tests[j].withdrawAmt, remainder)).add(increment.mul(BigNumber.from(tests[j].time + Number(j!=0)))) // +1s are to account for withdraw tx incrementing time by 1
